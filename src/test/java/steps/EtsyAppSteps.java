...
public class EtsyAppSteps {
    ...
    public void user_navigates_to_the_Etsy_application() {
     driver.get(ConfigReader.getProperty("EtsyAppsURL"));
   ...
   for(int i =0; i<itemNames.size(); i++){
      String newItemNames = itemNames.get(i).getText();//looping through the
       // List of WebElements and getting
       // text from each WebElement
      ...
      if(condition) {
          ...
          str.append(newItemNames);//each time it will add one more string from newItemNames until the List of
          // WebElements has elements

          System.out.println(newItemNames);
          Assert.assertTrue(newItemNames.toLowerCase().contains(item1) ||
                  newItemNames.toLowerCase().contains(item2));
      }
    ...
    public void userValidatesTitle(String expectedTitle) {
